	...
	for ptr_state in all_states:
		...
		for i in range(len(ptr_state)):
			...
			if not all_sensor_on:
				if belief_m and not all_sensor_on:
				#print(ptr_state)
					belief_sleep=0
					belief_wTV=0
					belief=0
...
def save_excel(Accuracy_list,F1_Sleep_list,F1_wTV_list,Energy_list,ws):
	
	ws.insert_cols(2, 4)
	ws.cell(column=2, row=1).value = 'Accuracy(gamma='+str(gamma)+')'
	ws.cell(column=3, row=1).value = 'F1-score_Sleep(gamma='+str(gamma)+')'
	ws.cell(column=4, row=1).value = 'F1-score_Watch TV(gamma='+str(gamma)+')'
	ws.cell(column=5, row=1).value = 'Energy Rate(gamma='+str(gamma)+')'
	for t in range(num_iteration+1):
		if t!=num_iteration:
			ws.cell(column=1, row=t+2).value = t+1
		else:
			ws.cell(column=1, row=t+2).value = "Max"
		ws.cell(column=2, row=t+2).value =Accuracy_list[t]
		ws.cell(column=3, row=t+2).value =F1_Sleep_list[t]
		ws.cell(column=4, row=t+2).value =F1_wTV_list[t]
		ws.cell(column=5, row=t+2).value =Energy_list[t]
def w1_r_save_excel(Accuracy,ws):
	
	ws.cell(column=int(Wer/0.1)+2, row=int(d/0.1)+2).value = Accuracy
...
def train(pick):
	...
	print("r=",1-r)
	jump=False
	pick_acc,p_F1_S,p_F1_W,p_en=pick
	for e in range(num_iteration):

		num_turn_on_off_sensor=0
		print(e+1)
		f = open('./'+str(data)+'/value/value r '+str(r)+'.txt','r')
		real_sensor=f.readline()
		fla = open('./'+str(data)+'/label/label r '+str(r)+'.txt','r')
		label=fla.readline()
		count=0
		hit=0
		all_en=0
		...
		for	t in range(0,86400):
			...
			if reliable:
				...
				if not all_sensor_on or (all_sensor_on and curr_state!="(0 1 0 1 0)" and curr_state!="(0 1 2 1 0)" and curr_state!="(0 1 0 1 2)" and curr_state!="(0 1 2 1 2)"and curr_state!="(2 1 2 1 2)"and curr_state!="(2 1 0 1 0)"and curr_state!="(2 1 2 1 0)"and curr_state!="(2 1 0 1 2)"):
				#if True:
					if belief_sleep<belief_wTV and belief_wTV>lwb:
						count+=1
						if "watch TV"in label:
							hit+=1
							pWaW+=1
		...
		if not jump:
			...
			if pWaW!=0:
				...
				beta=1
				F1_W=0
			if pick_acc<Accuracy:
				pick_acc=round(Accuracy,4)
				p_F1_S=round(F1_S,4)
				p_F1_W=round(F1_W,4)
				p_en=round(Energy,4)
		print(pSaS,pWaW)
		...
		F1_Sleep_list.append(round(F1_S,4))
		F1_wTV_list.append(round(F1_W,4))
		Energy_list.append(round(Energy,4))
	Accuracy_list.append(round(pick_acc,4))
	F1_Sleep_list.append(round(p_F1_S,4))
	F1_wTV_list.append(round(p_F1_W,4))
	Energy_list.append(round(p_en,4))
	return Accuracy_list,F1_Sleep_list,F1_wTV_list,Energy_list,(pick_acc,p_F1_S,p_F1_W,p_en)
if __name__ == '__main__':
	...
	if save_results:
		...
		fn = './Results_w1/MDP/'+str(data)+'/'+str(1-r)+'.xlsx'
		# fn = './Results_w1/case3/'+str(data)+'/'+str(1-r)+'.xlsx'
		# fn = './Results_new/MDP/d.xlsx'
		# fn='w1 and r.xlsx'
		# wb =load_workbook(fn)
		...
		for i in range(9):
			...
			Accuracy_list,F1_Sleep_list,F1_wTV_list,Energy_list,pick=train(pick)
			Cohort=copy.deepcopy(init_Cohort)
			
			save_excel(Accuracy_list,F1_Sleep_list,F1_wTV_list,Energy_list,ws)
		# w1_r_save_excel(pick[1],ws)
		wb.save(fn)
	else:
		print('gamma='+str(gamma))
		train(pick)
